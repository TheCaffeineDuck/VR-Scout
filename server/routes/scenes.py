...
async def get_scenes() -> list[SceneRow]:
...
async def create_new_scene(body: SceneCreate) -> SceneRow:
    """Create a new scene."""
    _validate_scene_id(body.id)
    existing = await get_scene(body.id)
    if existing is not None:
        raise HTTPException(status_code=409, detail=f"Scene '{body.id}' already exists")

    # Create scene directory
    scene_dir = settings.scenes_path / body.id
...
async def update_config(scene_id: str, config: SceneConfig) -> SceneRow:
    """Update scene config."""
    _validate_scene_id(scene_id)
    scene = await get_scene(scene_id)
    if scene is None:
...
async def update_alignment(scene_id: str, body: AlignmentUpdate) -> dict[str, str]:
    ...
    alignment_path = settings.scenes_path / scene_id / "alignment.json"
...
async def get_cameras(scene_id: str) -> dict[str, object]:
    ...
    _validate_scene_id(scene_id)
    cameras_path = settings.scenes_path / scene_id / "cameras.json"
